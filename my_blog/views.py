...
def postlist(request):
    if request.GET.get('search'):
        ...
        posts = PostModel.objects.all().order_by('-created_date')
...
def postupdate(request, p_id):
    ...
    if request.method == "POST":
        ...
        post.save()
        messages.warning(request, 'Post Update Success!')
...
def cmtupdate(request, p_id, c_id):
    ...
    if request.method == "POST":
        ...
        cmt.save()
        messages.warning(request, 'Your comment is successfully updated')
        return redirect (f'/blog/detail/{p_id}/#cmt')
